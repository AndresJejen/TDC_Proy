...
float reference = total_size/2 - 5; //poner en la mitad del tubo
...
float ueq = 17; // control para el equilibrio
float eh = 3;  // valor en centimetros del error de histeresis
float ud = 2;  // valor en voltios del cambio en la señal de control

// valores de encendido y apagado de la señal de control
float uon = ueq + ud - 0.5;
float uoff = ueq - ud; 
...
static void controlTask(void *pvParameters) {
    ...
    for (;;) {
       ...
       y = calibration-((float) sensor.readRangeContinuousMillimeters()/10 ) ;// control con histeresis
      ...
      if (time <= 120){
         printf(">Time: %0.3f, ref: %0.3f, ref_top: %0.3f, ref_bottom: %0.3f, u: %0.3f, y: %0.2f\n", time, reference, reference + eh, reference - eh, u, y);
         n+=1;
       }
